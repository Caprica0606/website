...
final class PagesAdmin extends Pages
{
  ...
  */
  public function listSubPagesSelectAdmin( $iPageSelected, $iPageParent, $iDepth = 1 ){
    ...
    while( $aData = $oQuery->fetch( PDO::FETCH_ASSOC ) ){
      $content .= '<option'.( $aData['iStatus'] == 0 ? ' style="color:#bbb;"' : null ).' value="'.$aData['iPage'].'">'.$sSeparator.$aData['sName'].'</option>'.$this->listSubPagesSelectAdmin( $iPageSelected, $aData['iPage'], $iDepth + 1 );
    } // end while
    return $content;
